...
class GalleryTermController extends WechatController {
    ...
    public function index(){
        ...
        $tree->nbsp = '&nbsp;&nbsp;&nbsp;';
        foreach ($result as $r) {
            $r['str_manage'] = '<a href="' . U("add", array("parent" => $r['term_id'])) . '">添加子类</a> | <a href="' . U("edit", array("id" => $r['term_id'])) . '">'.L('EDIT').'</a> | <a class="js-ajax-delete" href="' . U("delete", array("id" => $r['term_id'])) . '">'.L('DELETE').'</a> ';
            $url=U('V1/list/index',array('id'=>$r['term_id']));
            $r['url'] = $url;
        ...
        $str = "<tr>
					...
					<td>\$id</td>
	    			<td><img src='/data/upload/\$thumb' alt='' width='100px'></td>
					<td>\$spacer \$name</td>
					<td>\$str_manage</td>
				</tr>";
    ...
    public function add(){
        ...
        $tree->icon = array('&nbsp;&nbsp;&nbsp;│ ', '&nbsp;&nbsp;&nbsp;├─ ', '&nbsp;&nbsp;&nbsp;└─ ');
        $tree->nbsp = '&nbsp;&nbsp;&nbsp;';
        $terms = $this->terms_model->where("wid={$this->wechat_config['id']}")->order(array("path"=>"asc"))->select();
    ...
    public function add_post(){
        if (IS_POST) {
            if ($this->terms_model->create()!==false) {
                $this->terms_model->wid=$this->wechat_config['id'];
