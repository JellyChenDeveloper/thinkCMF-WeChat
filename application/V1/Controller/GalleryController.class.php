...
class GalleryController extends WechatController {
    ...
    function _initialize() {
        parent::_initialize();
        $this->posts_model = D("V1/Posts");
        $this->terms_model = D("V1/Terms");
        $this->term_relationships_model = D("V1/TermRelationships");
    }

    // 后台文章管理列表
    public function index(){
        $this->_lists(array("wid"=>$this->wechat_config['id'],"post_status"=>array('neq',3)));
        $this->_getTree();
        $this->display();
    }
    ...
    public function add(){
        ...
        $this->_getTermTree();
        $term=$this->terms_model->where(array('wid'=>$this->wechat_config['id'],'term_id'=>$term_id))->find();
    ...
    public function add_post(){
        if (IS_POST) {
            ...
            $article['wid']=$this->wechat_config['id'];
            $article['term_id']=I('term');
...
//                }
    ...
    public function edit(){
        ...
        $term_relationship = M('v1GalleryTerms')->where(array("wid"=>$this->wechat_config['id'],"status"=>1))->getField("term_id",true);
...
//
//            $this->term_relationships_model->where(array("wid"=>$this->wechat_config['id'],"object_id"=>$post_id,"term_id"=>array("not in",implode(",", $_POST['term']))))->delete();
//            foreach ($_POST['term'] as $mterm_id){
...
//            }
    ...
    private function _getTree(){
        ...
        $result = $this->terms_model->where(array("wid"=>$this->wechat_config['id'],"status"=>1))->order(array("listorder"=>"asc"))->select();
    ...
    private function _getTermTree($term=array()){
        $result = $this->terms_model->where(['wid'=>$this->wechat_config['id']])->order(array("listorder"=>"asc"))->select();
